...
import { asOwner, inRollbackTx, inSavepoint, withTenant, expectInvariantViolation, expectDbRejection } from "./helpers";
import {
  insertTenant,
  createSistemaUser,
...
async function insertAsientoSimple(
  ...
  const cols = ["tenant_id", "origen", "preparacion_id", "paciente_texto", "medico_texto", "formula_texto", "registrado_por_id"];
  const vals: unknown[] = [input.tenantId, "SISTEMA", input.preparacionId, "Paciente", "Medico - MAT-1", "Formula", input.sistema];
...
describe.skipIf(dbTestSkipReason() !== null)("0014_libro_recetario_contralor migration -- libro recetario (fsj schema)", () => {
  ...
  it("hash chain: recomputing hash_integridad in plain SQL from the stored fields matches the trigger-computed value", async () => {
    await asOwner((client) =>
      inRollbackTx(client, async (tx) => {
        ...
        const recomputed = recomputeAsientoHash({
        ...
        // Also rebuild the V2 payload by hand in plain SQL from the documented
        // format (length-prefixed fields, '-' for NULL, no separator) and
        // hash it with the DB's own digest() -- independent of both the Node
        // reimplementation above and fsj.hash_v2().
        const campo = (col: string) => `CASE WHEN ${col} IS NULL THEN '-' ELSE octet_length(convert_to(${col}, 'UTF8'))::text || ':' || ${col} END`;
        const dbRecomputed = await tx.query(
          `SELECT encode(extensions.digest(convert_to(
             ${campo("'FSJ-ASIENTO-V2'::text")} || ${campo("tenant_id::text")} || ${campo("libro_id::text")} ||
             ${campo("numero_correlativo::text")} || ${campo("to_char(fecha_asiento, 'YYYY-MM-DD')")} || ${campo("origen::text")} ||
             ${campo("preparacion_id::text")} || ${campo("asiento_original_id::text")} || ${campo("paciente_texto")} ||
             ${campo("medico_texto")} || ${campo("formula_texto")} || ${campo("hash_anterior")}, 'UTF8'), 'sha256'), 'hex') AS recomputed
           FROM fsj.asiento_recetario WHERE id = $1`,
          [seed.asientoId],
        );
        expect(dbRecomputed.rows[0].recomputed).toBe(r.hash_integridad);
      }),
    );
  });
  ...
  it("hash chain: tampering is detectable -- recomputing with an altered field does NOT match the stored hash", async () => {
    await asOwner((client) =>
      inRollbackTx(client, async (tx) => {
        ...
        // Recompute as if formula_texto had been tampered with -- must NOT match.
    ...
    await asOwner((client) =>
      inRollbackTx(client, async (tx) => {
        ...
        const dtUser = await tx.query(
          `INSERT INTO fsj.usuario (tenant_id, email, nombre, apellido, dni, creado_por_id) VALUES ($1,$2,'D','T',$3,$4) RETURNING id`,
  ...
  it("rectificativo in a SIGNED jornada works, gets the next correlativo, and a second rectificativo is rejected (INV-L18/L19)", async () => {
    await asOwner((client) =>
      inRollbackTx(client, async (tx) => {
        // Start the tenant in an "early" zone so today's jornada is well
        // defined, sign it, then jump the tenant's zona_horaria far enough
        // forward (Etc/GMT+12 -> Pacific/Kiritimati is a 26h offset swing)
        // that fsj.jornada_actual() moves to a LATER calendar day at the
        // very same real instant -- this is how these tests get a second,
        // strictly-later jornada without waiting for real time to pass.
        ...
        // Jump the tenant's jornada forward.
  ...
  it("V2 hash does not depend on the session DateStyle (trigger-computed hash under 'SQL, DMY' matches the spec recomputation)", async () => {
    await asOwner((client) =>
      inRollbackTx(client, async (tx) => {
        ...
        const hashSql = await tx.query(`SELECT fsj.asiento_recetario_hash_v2(a) AS h FROM fsj.asiento_recetario a WHERE id = $1`, [inserted.id]);

        await tx.query(`SET LOCAL DateStyle = 'ISO, YMD'`);
        const hashIso = await tx.query(`SELECT fsj.asiento_recetario_hash_v2(a) AS h FROM fsj.asiento_recetario a WHERE id = $1`, [inserted.id]);
        ...
        expect(inserted.hashIntegridad).toBe(spec);
    ...
    await asOwner((client) =>
      inRollbackTx(client, async (tx) => {
        ...
        await inSavepoint(tx, async () => {
          ...
          await tx.query(
            `UPDATE fsj.asiento_recetario a SET hash_integridad = fsj.asiento_recetario_hash_v2(a) WHERE id = $1`,
            [a2.id],
          );
          await tx.query(`SET LOCAL session_replication_role = origin`);
...
describe.skipIf(dbTestSkipReason() !== null)("0018_legal_core_fixes -- N2/N3 fsj.jornada_de (fsj schema)", () => {
  ...
  it("fsj.jornada_actual delegates to fsj.jornada_de(now(), tenant.zona_horaria)", async () => {
    await asOwner((client) =>
      inRollbackTx(client, async (tx) => {
        ...
        expect(def.rows[0].d).toContain("fsj.jornada_de(now()");
      }),
    );
  });
});
