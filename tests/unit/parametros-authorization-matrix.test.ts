/**
 * Authorization matrix test for FASE 3 point 3.10b (modules/parametros/**).
 * Same pattern as tests/unit/farmacia-authorization-matrix.test.ts (which
 * see for the full rationale) -- separate file per the task instruction to
 * keep the two concerns (tenant data vs parametros) clearly separated.
 */
...
await import("@/modules/parametros/application/list-parametros");
await import("@/modules/parametros/application/editar-parametro");
...
const CASES: ReadonlyArray<{ name: string; permiso: Permiso; input: unknown }> = [
  { name: "parametros.list", permiso: "config.ver", input: {} },
  { name: "parametros.editar", permiso: "config.editar", input: { clave: "precision_balanza", valor: "0.01" } },
];
...
describe("FASE 3.10b (modules/parametros) authorization matrix -- declared permiso", () => {
...
describe("FASE 3.10b (modules/parametros) authorization matrix -- role x permiso, exercised through the REAL execute() path", () => {
