...
/** Minimal Server Action form wrapper for `/admin/precios` and `/recetas/[id]/items/[itemId]/cotizacion` -- own copy, see modules/stock/ui/simple-form.tsx for the shared shape. */
...
export interface SimpleFormProps {
  action: (prevState: PreciosActionState, formData: FormData) => Promise<PreciosActionState>;
  children: ReactNode;
  submitLabel: string;
  pendingLabel?: string;
  ...
  return (
    <form action={formAction} noValidate className={className}>
      ...
      {state.status === "success" && state.message ? (
        <p role="status" className="mt-2 text-sm text-green-700 dark:text-green-500">
          {state.message}
        </p>
      ) : null}
      <div className="mt-3">
